...
@Component({
selector: 'app-post',
templateUrl: './post.component.html', styleUrls: ['./post.component.sass']
})
...
// instanciando nosso provider de blog public blog: BlogService) {
// chama a rota ativa this.activatedRoute
.params
// se inscreve para quando for resolvida
.subscribe(
// recebe os parametros params => {
// declara uma variável id baseada no parametro id da url let id = params['id'];
// pego o post em questão this.blog.post(id)
// quando resolvido
.then((post: any) => {
// liga os dados na view (template) this.post = post;
})
// tratamento de erro
.catch((e) => { console.error(e);
// força navegação para página 404 this.router.navigate(['/404']);
})
});
}
ngOnInit() { }
